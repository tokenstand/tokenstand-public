...
import RemoveTransactionMobile from "./Mobile/RemoveTransactionMobile";
import { client } from "../../services/graph/apollo/client";
import { queryRemoveTransaction } from "../../services/graph/queries/exchange";
import {
  linkTransaction,
  handleValue,
...
let transactions = [];
export default function AddTransaction({ tabIndex, ethPrice }) {
  ...
  const getRemoveTransactions = async () => {
    try {
      let result = await client(chainId).query({
        query: queryRemoveTransaction,
        variables: {
          account: account,
        },
        fetchPolicy: "cache-first",
      });
      transactions = result?.data?.burns;
      setRemoveHistory(transactions);
      setLoading(false);
      setTableOptions({
        ...tableOptions,
  ...
  useEffect(() => {
    getRemoveTransactions();
  }, [tabIndex]);

  const link = linkAddress(chainId, account);

  return removeHistory && removeHistory.length > 0 ? (
    isMobile ? (
      <RemoveTransactionMobile
      ...
      <>
        <TableRouting isMobile={isMobile}>
          <div className="text-left  color-gray">Remove</div>
          ...
          {result.map((item) => (
            <>
              <div>
                ...
                >
                  {" "}
                  Remove {item.token0.symbol} for {item.token1.symbol}
                </LinkHistory>
              </div>
    ...
    <div className="flex flex-col items-center justify-center py-12">
      {loading ? (
        ...
        <>
          <TextWarning>
            Can&apos;t find removes transactions for address
          </TextWarning>
