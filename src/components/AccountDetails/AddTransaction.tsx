...
import AddTransactionMobile from "./Mobile/AddTransactionMobile";
import { client } from "../../services/graph/apollo/client";
import { queryAddTransaction } from "../../services/graph/queries/exchange";
import {
  linkTransaction,
  handleValue,
...
export default function AddTransaction({ tabIndex, ethPrice }) {
  ...
  const getAddTransactions = async () => {
    try {
      let result = await client(chainId).query({
        query: queryAddTransaction,
        variables: {
          account: account,
        },
        fetchPolicy: "cache-first",
      });
      transactions = result?.data?.mints;
      setAddHistory(transactions);
      setLoading(false);
      setTableOptions({
        ...tableOptions,
  ...
  return addHistory && addHistory.length > 0 ? (
    isMobile ? (
      ...
      <>
        <TableRouting isMobile={isMobile}>
          <div className="text-left  color-gray">Add</div>
          ...
          {result.map((item) => (
            <>
              <div>
                ...
                >
                  {" "}
                  Add {item.token0.symbol} and {item.token1.symbol}
                </LinkHistory>
              </div>
    ...
    <div className="flex flex-col items-center justify-center py-12">
      {loading ? (
        ...
        <>
          <TextWarning>
            Can&apos;t find adds transactions for address
          </TextWarning>
