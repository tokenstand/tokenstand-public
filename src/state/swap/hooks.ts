...
export function useDerivedSwapInfo(doArcher = false): {
  ...
  let inputCurrency = useCurrency(inputCurrencyId || getDefaultOutput(Number(chainId)))
  let nativeCurrency = useCurrency('0x0000000000000000000000000000000000000000')
  if(!inputCurrency&&inputCurrencyId){
    inputCurrency = nativeCurrency
  }
  let outputCurrency = useCurrency(outputCurrencyId || getDefaultOutput(Number(chainId)))
...
export const getDefaultOutput = (chainId: number) => {
